...
import { X, Upload, FileSpreadsheet, Trash2, Download, AlertCircle, Pencil, Check, Star } from 'lucide-react';
...
export default function TemplateImportModal({ isOpen, onClose, onApplyTemplate }: TemplateImportModalProps) {
  const { user } = useAuth();
      ...
      reader.onload = (e) => {
        try {
          ...
          const normalizedData = jsonData.map((row: any, index: number) => {
            const normalized = {
              ITEM: findValue(row, ['ITEM', 'item', 'Item']),
              ETAPA: findValue(row, ['ETAPA', 'etapa', 'Etapa']),
              Descrição: findValue(row, [
                'Descrição', 'DESCRIÇÃO', 'Descricao', 'DESCRICAO', 'descricao',
                'Descricão', 'Description', 'description', 'Descriçao'
              ]),
              MACRO: findValue(row, ['MACRO', 'macro', 'Macro']),
              UNIDADE: findValue(row, ['UNIDADE', 'unidade', 'Unidade', 'Und', 'UND']),
              QUANTIDADE: findValue(row, ['QUANTIDADE', 'quantidade', 'Quantidade', 'QTD', 'qtd', 'Qtd']),
              'VALOR UNIT': findValue(row, [
                'VALOR UNIT', 'VALOR_UNIT', 'valor_unitario', 'valor unitario',
                'Valor Unit', 'Valor Unitário', 'VALOR UNITÁRIO'
              ]),
              TOTAL: findValue(row, ['TOTAL', 'total', 'Total']),
              ORÇAMENTO: findValue(row, ['ORÇAMENTO', 'ORCAMENTO', 'orcamento', 'Orçamento', 'Orcamento']),
  ...
  const handleDelete = async (templateId: string) => {
    ...
    try {
      ...
      if (template?.arquivo_url) {
        const urlParts = template.arquivo_url.split('/');
  ...
  const handleDownload = async (template: BudgetTemplate) => {
    try {
      ...
      a.href = url;
      a.download = template.arquivo_nome;
      ...
      alert('Erro ao fazer download do template');
    }
  };

  const handleEditDescription = (template: BudgetTemplate) => {
    setEditingId(template.id);
    setEditingDescription(template.descricao || '');
  };
  ...
  const handleSaveDescription = async (templateId: string) => {
    ...
    try {
      const { error } = await supabase
        .from('budget_templates')
        .update({ descricao: editingDescription.trim() })
        .eq('id', templateId);
  ...
  const handleApplyTemplate = async (template: BudgetTemplate) => {
    if (!onApplyTemplate) {
      alert('Funcionalidade de aplicar template não disponível neste contexto');
      return;
    }
    ...
    try {
      ...
      if (Array.isArray(itens) && itens.length > 0) {
        ...
        alert('Template não contém itens para aplicar');
      }
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-2xl font-bold text-gray-800">Templates de Orçamento</h2>
          <button
            onClick={onClose}
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={selectedFiles.length > 1 ? "Prefixo para os templates (ex: Acabamento)" : "Ex: Template Básico, Template Executivo..."}
                />
            ...
            <div className="space-y-4">
              {loading ? (
                ...
                <div className="text-center py-12">
                  <FileSpreadsheet size={64} className="mx-auto text-gray-900 mb-4" />
                  ...
                  >
                    <div className="flex items-center gap-4 flex-1">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-500">{template.arquivo_nome}</p>
