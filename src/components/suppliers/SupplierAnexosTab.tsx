...
interface SupplierAnexosTabProps {
  supplierId: string | null;
}

export default function SupplierAnexosTab({ supplierId }: SupplierAnexosTabProps) {
  const { user } = useAuth();
  ...
  const handleCreateFolder = async () => {
    if (!newFolderName.trim() || !supplierId || !user) return;
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        const fileName = `${user.id}/${supplierId}/${Date.now()}_${i}.${fileExt}`;
  ...
  const handleDeleteAttachment = async (attachment: SupplierAttachment) => {
  ...
  if (!supplierId) {
    return (
      <div className="text-center py-8 text-gray-500">
        Salve o fornecedor primeiro para adicionar anexos
      </div>
    );
  }
